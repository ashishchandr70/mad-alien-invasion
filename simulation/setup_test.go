...
func TestWorldLargeFile(t *testing.T) {
	testWorldReadFromFile(t, "../inputs/worldmap_large.txt", 6763)
}

func TestWorldFile(t *testing.T) {
	testWorldReadFromFile(t, "../inputs/worldmap.txt", 148)
}

func testWorldReadFromFile(t *testing.T, file string, num int) {
	w, input, err := LoadWorldMapFromFile(file)
	if err != nil {
...
func testRandAliens(t *testing.T, n int, seed int64) {
	...
	r := rand.New(source)
	aliens := CreateAliensFromRandomSeed(n, r)
	if len(aliens) != n {
		t.Errorf("len(CreateAliensFromRandomSeed(%d, 0xffffffff)) = %d; want %d", n, len(aliens), n)
		return
	}
	fmt.Printf("Aliens:\n%s\n", aliens)
}
